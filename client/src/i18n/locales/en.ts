const en = {
  global: {
    title: "OTT PAY SYSTEM",
    home: "Home",
    dashboard: "Dashboard",
  },
  settings: {
    login: "Login",
    logout: "Logout",
    logoutConfirm: "Are you sure you want to logout?",
  },
  login: {
    systemTitle: "Admin System",
    heroTitle1: "Manage your",
    heroTitle2: "Digital Workspace",
    heroTitle3: "with confidence.",
    heroSubtitle: "Experience total control with dynamic routing, advanced role-based security, and global language support. Modern administration, simplified.",
    copyright: "© 2026 Admin System by Zihang Cheng",
    welcomeTitle: "Hello,",
    welcomeSubtitle: "Welcome Back!",
    credentialsPrompt: "Please enter your credentials to continue.",
    usernamePlaceholder: "Username or Email",
    usernameRequired: "Please input your Username/Email!",
    passwordPlaceholder: "Password",
    passwordRequired: "Please input your Password!",
    rememberMe: "Remember me",
    forgotPassword: "Forgot Password?",
    demoAccess: "Demo Access",
    adminAccount: "Admin Account",
    fullPermissions: "Full access permissions",
  },
  fetch: {
    errorCode: {
      201: "Successfully created or modified data.",
      202: "A request has entered the background queue (asyn task).",
      204: "Data deleted successfully.",
      400: "There is an error in the request, the server has not performed the operation of creating or modifying data.",
      401: "User does not have permission (token, user name, password error).",
      403: "The user is authorized, but access is prohibited.",
      404: "Request sent is for a nonexistent record, and the server did not operate.",
      406: "Request format is not available.",
      410: "Request resource is permanently deleted and will no longer be available.",
      412: "Client request information precondition error.",
      422: "A validation error occurred while creating an object.",
      500: "An error occurred on the server, Please check the server.",
      502: "Gateway error.",
      503: "Service is unavailable, the server is overloaded or under maintained.",
      504: "Gateway timeout.",
    },
    errorText: "Request timeout.",
    lockText: "Lock request.",
    cancelText: "Cancel request.",
    networkErr: "Network Error.",
  },
  button: {
    confirm: "Confirm",
    cancel: "Cancel",
    close: "Close",
    submit: "Submit",
    remove: "Remove",
    add: "Add",
    edit: "Edit",
    modify: "Modify",
    delete: "Delete",
    details: "Details",
    view: "View",
    print: "Print",
    export: "Export",
    import: "Import",
    upload: "Upload",
    download: "Download",
    search: "Search",
    reset: "Reset",
    save: "Save",
    clear: "Clear",
    operation: "Operation",
    preview: "Preview",
    reset_pwd: "Reset Password",
  },
  information: {
    title: "Prompt information",
    confirm: "Confirm the operation?",
    success: "Setting success!",
    failed: "Setting failed!",
  },
  menu: {
    dashboard: {
      main: "Dashboard",
    },
    projects: {
      main: "Projects",
    },
    systemManagement: {
      main: "System Management",
      user: "User Management",
      role: "Role Management",
    },
  },
  message: {
    normal_success: "Operation Successful!",
    normal_failed: "Operation Failed!",
    del_success: "Delete Successful!",
    del_failed: "Delete Failed!",
    reset_pwd_success: "Password Reset Successful!",
  },
  modal: {
    infoModal: {
      del_title: "Delete",
      del_msg: "Are you sure to delete this data?",
      confirm_title: "Notice",
      reset_pwd_msg: "Are you sure to reset this user's password?",
    },
  },
  text: {
    addUser: "Add User",
    editUser: "Edit User",
    addRole: "Add Role",
    editRole: "Edit Role",
    operation: "Operation",
    password: "Password",
    confirmPsw: "Confirm Password",
  },
  page: {
    userManagement: {
      username: "Username",
      status: "Status",
      role: "Role",
      createdAt: "Created Time",
      updatedAt: "Update Time",
      email: "Email",
    },
    roleManagement: {
      roleName: "Role Name",
      description: "Description",
      createdAt: "Created Time",
      updatedAt: "Update Time",
      rolePlaceholder: "Please enter role name",
      descriptionPlaceholder: "Please enter description",
      permissionConfig: "Permission Configuration",
      selectAllRoutes: "Select All Routes",
      deselectAllRoutes: "Deselect All Routes",
      allRoutesReadOnly: "Set All Selected Routes to Read-Only",
      allRoutesWrite: "Set All Selected Routes to Read and Write",
      readOnly: "Read Only",
      readWrite: "Read & Write",
    },
  },
};
export default en;
